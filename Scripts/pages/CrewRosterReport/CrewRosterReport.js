...
function dateRangeChange() {
    ...
    $("#TxtFromDate").val(fromDate).change();
    $("#TxtToDate").val(toDate).change();
...
function LoadCrewReportGrid() {
    ...
    $("#CrewRosterReport").jqGrid({
        url: applicationUrl + "CrewRosterReport/GetCrewRosterReport",
        datatype: "json",
        mtype: 'POST',
        postData: {
            ...
            staffId: function () {
                if ($("#EmployeeName").val()) {
                    ...
                    // alert("emp" + $("#EmployeeName").val())
                }
                else {
                    // alert("staff" + $("#sltStaffID").val())
                    return $("#sltStaffID").val();
                }
        ...
        colNames: ['Duty Date', 'Staff ID', 'Employee Name', 'Rank', 'Employee Category', 'Base', 'Planned Duty', 'Actual Duty', 'Changes', 'Notification Time Gap', 'Reporting start time', 'Changed On', 'Changed By', 'Notified On', 'NotifiedBy', 'Remarks'],
        colModel: [
            { hidden: false, name: 'DutyDate', index: new Date('DutyDate'), align: "center" },
        ...
        rowNum: 100,
        rownumbers: true,
        rowList: [10,50, 100,1000,5000,10000],
        height: 400,
        width: '100%',
        autowidth: true,
        loadonce: true,
...
function GetBaseDD() {
    ...
    $.ajax({
        type: 'POST',
        url: applicationUrl + "CrewRosterReport/GetBaseListForDD_rpt",
        dataType: 'json',
        data: {
            fromDate: function () { return $("#TxtFromDate").val(); },
            toDate: function () { return $("#TxtToDate").val(); },
...
function loadRankWithSignleDD() {
    ...
    $.ajax({
        type: 'POST',
        url: applicationUrl + "CrewRosterReport/Get_RankListForDD_rpt",
        dataType: 'json',
        data: {
            fromDate: function () { return $("#TxtFromDate").val(); },
            toDate: function () { return $("#TxtToDate").val(); },
...
function GetStaffIdAndEmployeeName() {
    ...
    // staffid
    /* commented by Pankaj on  30 OCT 2022
     * $.ajax({
        type: 'POST',
        url: applicationUrl + "CrewRosterReport/Get_StaffListForDD_rpt",
        dataType: 'json',
        data: {
            fromDate: function () { return $("#TxtFromDate").val(); },
            toDate: function () { return $("#TxtToDate").val(); },
    ...
    $.ajax({
        type: 'POST',
        url: applicationUrl + "CrewRosterReport/Get_EmployeeNameForDD_rpt",
        dataType: 'json',
        data: {
            fromDate: function () { return $("#TxtFromDate").val(); },
            toDate: function () { return $("#TxtToDate").val(); },
        ...
        success: function (data) {
            console.log("staff id EmployeeName", data)
            type = [];
            for (var j = 0; j < data.length; j++) {
                type.push(data[j])
            }
...
// for Employee Name
            ...
            // for StaffID
            for (var j = 0; j < type.length; j++) {
                $('#sltStaffID').append('<option value="' + type[j].Id + '">' + type[j].Id + '</option>');
            }
            $('#sltStaffID').multiselect({
                includeSelectAllOption: true,
                enableFiltering: true,
                ...
                nonSelectedText: 'Select Employee Name',
                onChange: function () {
                    console.log('sltStaffID lib chnage', $("#EmployeeName").val());
                    if ($("#sltStaffID").val() != null) {
                        $("#EmployeeName").multiselect("disable");
                    } else {
                        $("#EmployeeName").multiselect("enable");
